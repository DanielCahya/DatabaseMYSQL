...
('R', 'Romi'),
('S', 'Sugeng');
INSERT INTO tblJadwal (kodeJadwal, hari, jam) VALUES
(1,'SENIN', '08.00-10.00'),
(2,'SENIN', '10.00-12.00'),
(3,'SENIN', '12.00-14.00'),
(4,'SENIN', '14.00-16.00'),
...
-- Create stored procedure to generate random jadwal
DELIMITER $$
...
BEGIN
  ...
  OPEN cMKU;
  SET i:=1;
  loopi: WHILE i <= vJumData DO
    FETCH cMKU INTO vKodeMKU, vJumlah;    
    SET j:=vMin;
    loopj: WHILE j <= vJumlah DO      
      IF vKodeMKU = 'K' THEN
        -- Assign Ali or Beni to Kewarganegaraan
        IF RAND() < 0.5 THEN
          SET vKodeDosen = 'A';
        ELSE
          SET vKodeDosen = 'B';
      ...
      ELSEIF vKodeMKU = 'P' THEN
        -- Assign Ali or Beni to Pancasila
        IF RAND() < 0.5 THEN
          SET vKodeDosen = 'A';
        ELSE
          SET vKodeDosen = 'B';
      ...
      ELSEIF vKodeMKU = 'R' THEN
        -- Assign Romi or Sugeng to Religiusitas
        IF RAND() < 0.5 THEN
          SET vKodeDosen = 'R';
        ELSE
          SET vKodeDosen = 'S';
      ...
      -- Randomly select a jadwal for the course
      SELECT FLOOR(RAND()*(vMax-vMin)+vMin) INTO vKodeJadwal;
      
      -- Check if the dosen is available at the selected time
...
-- Create trigger to prevent duplicate jadwal for the same dosen
DELIMITER $$
...
-- Call the stored procedure to generate random jadwal
CALL spRandomJadwal();

-- View the generated jadwal
SELECT 
